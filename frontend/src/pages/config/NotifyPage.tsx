...
import { listNotify, createNotify, deleteNotify } from '../../services/api'
  ...
  const columns = [
    ...
    { title: '状态', dataIndex: 'enabled', render: (v: boolean) => <Tag color={v ? 'green' : 'red'}>{v ? '启用' : '停用'}</Tag> },
    {
      title: '操作', render: (_: any, r: any) => (
        <Popconfirm title="确认删除？" onConfirm={() => deleteNotify(r.id).then(load)}>
  ...
  return (
    <div style={{ background: '#fff', padding: 24, borderRadius: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => { form.resetFields(); setOpen(true) }}>添加通知</Button>
      </div>
        ...
        <Form form={form} layout="vertical" initialValues={{ provider: 'feishu_webhook', is_default: false, enabled: true }}>
          ...
          {provider === 'feishu_webhook' && (
            <>
              ...
              <Form.Item name="sign_key" label="签名密钥 (可选)"><Input.Password /></Form.Item>
            </>
          )}
