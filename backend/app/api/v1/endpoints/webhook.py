...
from app.services.git.webhook_parser import parse_gitlab, parse_github, parse_gitea, _normalize_url
...
async def receive_webhook(
    ...
    # Find matching repository — normalize both sides to handle .git suffix differences
    all_repos = (await db.execute(
        select(Repository).where(Repository.platform == platform, Repository.enabled == True)
    )).scalars().all()
    repo = next(
        (r for r in all_repos if _normalize_url(r.repo_url) == event.repo_url),
        None,
    )
